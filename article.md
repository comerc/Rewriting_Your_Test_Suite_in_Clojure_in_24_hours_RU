...
Очевидно, что непрактично переписывать 5000 тестов руками. Вместо этого мы решили использовать Clojure, чтобы переписать их автоматически, используя встроенные в Clojure функции манипулирования языком.
...
Поведение `->` похоже на chaining в Ruby или JQuery, или как Bash’s pipes: передаёт результат вычисления вызова функции, как аргумент, в вызов следующей функции.
...
Интересный процесс происходит в функциях замены. Они все, как правило, касаются форм:
...
Большинство преобразований простые. Однако, всё становится гораздо сложнее с формой `contains`:
...
Последний кейс особенно интересный. В выражении
...
Была ещё одна дополнительная компиляция с автоматическим преобразованием. Если имеем два выражения:
